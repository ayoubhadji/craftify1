...
public class PostCardFactory {
    ...
    public static VBox createCard(Post post, int currentUserId, boolean isRecommended, Runnable onRefresh) {
        ...
        titleBox.setAlignment(Pos.CENTER); // center horizontally

        commentSection.getChildren().add(titleBox);


        try {
            List<Commentaire> commentaires = new PostService(MyDb.getInstance().getConnection()).getCommentairesByPostId(post.getIdPost());
            ...
            dialog.showAndWait().ifPresent(commentText -> {
                if (!commentText.trim().isEmpty()) {
                    ...
                    try {
                        ...
                        new PostService(MyDb.getInstance().getConnection()).addComment(newComment);
                        onRefresh.run();
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
